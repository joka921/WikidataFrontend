...
TEST(EntityFinderTest, rankResults) {
  ...
  // note: when an entity occurs in prefix and exact matches we do not eliminate
  // the duplicate
  ASSERT_EQ(1, prefix[0]._idx);
  ASSERT_EQ(0, prefix[1]._idx);

  // Mocking a search for "doug", 0 exact matches and 4 prefix matches, two of
  // them are duplicates and will be eliminated
...
TEST(EntityFinderTest, convertIdxVecs) {
  ...
  EntityFinder finder;
  auto converted =
      finder.convertIdxVecsToSearchResult(exactMatches, prefixMatches, v);
...
TEST(EntityFinderTest, initializeFromTextFile) {
  ...
  ASSERT_EQ(string("British author and humorist (1952–2001)"),
            ent._description);
