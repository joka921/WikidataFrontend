...
class ServerUtils {
 public:
  ...
  // Serialize the input arguments to json. Format will be res["entities"] = <json version of input>
  ...
  // parse a Prefix Search query that has the form "?t=obj?q=<prefix>", "?t=prd?q=<prefix>" or "?t=all?q=<prefix>"
  // <prefix> lands in the string of the result type and SearchMode is set according to "obj, prd, all".
  // Ill-formed queries will cause SearchMode::Invalid and an undefined result string.
  static std::pair<std::string, SearchMode> parsePrefixSearchQuery(const std::string &query);
