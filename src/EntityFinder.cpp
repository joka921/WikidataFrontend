...
void EntityFinder::InitializeFromTextFile(const std::string& filename) {
  ...
  std::sort(_subjectVecs._aliases.begin(), _subjectVecs._aliases.end(), sortPred);
...
std::vector<WikidataEntity> EntityFinder::findEntitiesByPrefix(string prefix, SearchMode mode)
{
  ...
  auto upperBoundPrefixesPred = [](const std::string& p1, const std::pair<std::string, unsigned>&  p2)
  ...
  if (lower == v._aliases.end() ||
   ...
   std::vector<WikidataEntity> ret = convertIdxVecsToSearchResult(sortedIndices.first, sortedIndices.second,
                                      v);
   return ret;
}
