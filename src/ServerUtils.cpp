...
QuerySettings ServerUtils::parseSetting(const std::string& settingStr) {
  QuerySettings res;
  if (settingStr.size() < 3) {
    return res;
  }
  ...
  switch (settingStr[1]) {
    case 'x' :
      res.type = OrderType::None;
      break;
    case 's' :
      res.type = OrderType::NumSitelinks;
      break;
    case 'n' :
      res.type = OrderType::Numeric;
      break;
    case 'a' :
      res.type = OrderType::Alphabetical;
      break;
    default:
      return res;
  }

  auto idxString = settingStr.substr(2);
  try {
    res.orderIdx = std::stoi(idxString);
  } catch (...) {
    return res;
  }
